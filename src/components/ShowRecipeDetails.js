...
class ShowRecipeDetails extends Component {
    ...
    axios
      .get(
        "http://localhost:8082/api/" +
          id +
          "?token=" +
          this.props.location.state.token.token
      )
      .then((res) => {
        this.setState({
          recipe: res.data,
    ...
    axios
      ...
      .then((res) => {
        ...
        this.props.navigate("/show-recipes", {
          state: this.props.location.state.token,
        });
      })
      .catch((err) => {
  ...
  render() {
    ...
    //   );
    var show =
      String(this.props.location.state.token.email) === this.state.recipe.email;
    ...
    return (
      <div className="ShowRecipeDetails">
        <div className="container">
          ...
          <div style={{ paddingBottom: "5%" }}>{RecipeItem}</div>
